...
class UndergroundSystem:
    ...
    def checkOut(self, id: int, stationName: str, t: int) -> None:
        if len(self.users[id]) == 1:
            use_t = t - self.users[id][0][1]
            path = (self.users[id][0][0], stationName)
            if path in self.paths:
                self.paths[path].append(use_t)
            else:
                self.paths[path] = [use_t]
        self.users.pop(id)

    def getAverageTime(self, startStation: str, endStation: str) -> float:
        return np.mean(self.paths[(startStation, endStation)])
